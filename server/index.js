...
// get feedback pour chaque client tous seul
...
const role="admin";
    db.query(
      `INSERT INTO user (name, password, email , telephone , societe, role) VALUES ('${name}','${hash}','${email}','${telephone}','${societe}','${role}')`,
      function (error, results) {
        if (error) throw error;
        res.send(results);
      }
...
app.put("/validercommercial/:id", (req, res) => {
  ...
  const body = req.body;
  console.log(id)
  const queryString =`UPDATE demande SET  remarque ="demande accepté" ,delai=? , encours=? WHERE id =?`

  db.query(queryString,[body.delai,body.encours,id],(err, result) => {
    if (err) {
      console.log(err);
      return;
    }
...
adresse,username,nom,activite,tele,ident,resp,cin,personne,rib,ca,mode,delai,encours,
  ...
  db.query(
    `INSERT INTO demande SET adherent='${adherent}',adresse='${adresse}',username='${username}',nom='${nom}',activite='${activite}',tele='${tele}',ident='${ident}',resp='${resp}',cin='${cin}',personne='${personne}',rib='${rib}',ca='${ca}',mode='${mode}',delai='${delai}',encours='${encours}',
    iduser=( 
      SELECT id
      FROM user
      WHERE name = '${username}'
